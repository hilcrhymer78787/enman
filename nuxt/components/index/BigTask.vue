<template>
    <div>
        <v-card v-for="(task,taskIndex) in bigTasks" :key="taskIndex" :class="task.works.length ? 'blue' : 'grey'" class="lighten-5 mb-4 px-2 pb-1 pt-2">
        ...
        <v-dialog v-model="dialog" scrollable>
            <v-card>
                ...
                <v-card-text>
                    <ul>
                        <li v-for="(n, i) in 20" :key="i">テキスト{{i}}</li>
                    </ul>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
...
export default {
    ...
    computed: {
        bigTasks() {
            return this.tasks.filter((task) => !task.is_everyDay);
        },
    },
    methods: {
